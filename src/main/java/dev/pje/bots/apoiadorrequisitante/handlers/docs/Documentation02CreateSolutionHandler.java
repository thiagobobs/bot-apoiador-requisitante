...
//										String commitMessage = "[RELEASE] Geração do release notes para o PJe versão 2.2.0.1";
...
//							mrResponse = gitlabService.openMergeRequestIntoBranchDefault(gitlabProjectId, branchName, "[RELEASE] Geração do release notes para o PJe versão " + releaseNotes.getVersion());
//							
//
...
//		}
	...
	private void atualizaDocsPje(String projectName, VersionReleaseNotes releaseNotes) {
		...
		if (conteudoArquivo == null) {
			messages.error(String.format("Arquivo %s não encontrado no branch %s do projeto %s", pathToReleaseNotesPage, GitlabService.BRANCH_MASTER, gitlabProjectId));
		} else {
			if (conteudoArquivo.contains(texto)) {
				messages.info(String.format("O conteúdo que se pretende incluir já consta no branch %s", GitlabService.BRANCH_MASTER));
			} else {
				...
				String commitMessage = String.format("[RELEASE] Geração do release notes para o projeto %s versão %s", projectName, releaseNotes.getVersion());
				...
				if (commitResponse != null) {
					messages.info("Conteúdo do arquivo " + pathToReleaseNotesPage + " atualizado no branch " + branchName + " do projeto " + gitlabProjectId);
				} else {
