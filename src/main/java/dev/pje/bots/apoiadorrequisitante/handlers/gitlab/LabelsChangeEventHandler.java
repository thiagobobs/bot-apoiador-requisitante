...
public class LabelsChangeEventHandler extends Handler<GitlabEventMergeRequest>{
	 ...
	 * 
	 * 1- Usuário não poderá excluir label(s).
	 * 
	 * 2- Usuário não poderá incluir mais de um label.
	 * 
	 * 3- Usuário somente poderá incluir label de aprovação se:
	 * 		3.1- Pertencer ao grupo revisor no Jira (PJE_RevisoresDeCodigo).
	 * 		3.2- Possuir email diferente do usuário que realizou o commit.
	 * 		3.3- A label pertencer ao tribunal em que está lotado.
	 */
	@Override
		...
		if (!email.equals("bot.revisor.pje@cnj.jus.br") && GitlabMergeRequestActionsEnum.UPDATE.equals(event.getObjectAttributes().getAction()) 
			...
			if (!changedLabels.getRemovedItems().isEmpty()) {
				this.gitlabService.sendMergeRequestComment(event.getProject().getId().toString(), event.getObjectAttributes().getIid(), 
						"Usuário " + email + " não possui permissão para a excluir label(s).");
				...
				if (addedItemsLabel.size() > 1) {
					this.gitlabService.sendMergeRequestComment(event.getProject().getId().toString(), event.getObjectAttributes().getIid(), 
							"Usuário " + email + " não possui permissão para a incluir mais de 1 (um) label.");
					...
					if (!jiraService.isRevisorCodigo(jiraUser)) {
						this.gitlabService.sendMergeRequestComment(event.getProject().getId().toString(), event.getObjectAttributes().getIid(), 
								"Usuário " + email + " não possui permissão para adicionar label de aprovação.");
						
						this.gitlabService.atualizaLabelsMR(event.getObjectAttributes(), GitlabUtils.translateGitlabLabelListToValueList(event.getChanges().getLabels().getPrevious()));
					} else {
						if (event.getObjectAttributes().getLastCommit().getAuthor().getEmail().equals(jiraUser.getEmailAddress())) {
							this.gitlabService.sendMergeRequestComment(event.getProject().getId().toString(), event.getObjectAttributes().getIid(), 
									"Usuário " + email + " não possui permissão para aprovar o próprio código.");
							...
							if (tribunalUsuario == null || !addedItemsLabel.get(0).getTitle().contains(tribunalUsuario)) {
								this.gitlabService.sendMergeRequestComment(event.getProject().getId().toString(), event.getObjectAttributes().getIid(), 
										"Usuário " + email + " não possui permissão para incluir label de aprovação de outro tribunal que não o seu.");
								...
								this.jiraService.adicionarComentario(issue, "MR#" + event.getObjectAttributes().getIid() +" aprovado pelo usuário " + jiraUser.getEmailAddress(), updateFields);
								...
								logger.info("Issue type: {} | Labels aprovação: {}", issue.getFields().getIssuetype().getName(), labelsAprovacao);
...
//					
//				}else {
//					messages.error("O usuário " + (revisorJira != null ? revisorJira.getName() : "NULL") + " não possui permissão para alterar uma ou mais labels do MR, revertendo alterações.");
//					
