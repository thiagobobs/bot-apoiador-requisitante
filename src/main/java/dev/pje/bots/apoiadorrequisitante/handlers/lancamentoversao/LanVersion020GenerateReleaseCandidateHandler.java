				...
				if(implementsGitflow) {
					if(StringUtils.isNotBlank(versaoAfetada)) {
						...
						if(!tagJaExistente) {
							...
							String jql = jiraService.getJqlIssuesFromFixVersion(versaoASerLancada, false, Collections.singletonList(issue.getFields().getProject().getKey()));
...
//		}
	...
	public List<ProcessingMessage> comunicatePreviewLaunch(String newVersion, Boolean test) {
		messages.clean();
		
		String releaseBranchName = GitlabService.BRANCH_RELEASE_CANDIDATE_PREFIX + newVersion;
		String jql = jiraService.getJqlIssuesFromFixVersion(newVersion, false, null);

		releaseCandidateModel.setProjectName("pje");
		releaseCandidateModel.setVersion(newVersion);
		releaseCandidateModel.setJql(jql);
		releaseCandidateModel.setBranchName(releaseBranchName);
		
		String mensagemRoketchat = this.gerarMensagemRocketchat();
		if(StringUtils.isNotBlank(mensagemRoketchat)) {
			rocketchatService.sendBotMessage(mensagemRoketchat);
		...
		String mensagemTelegram = this.gerarMensagemTelegram();
		if(StringUtils.isNotBlank(mensagemTelegram)) {
			telegramService.sendBotMessageHtml(mensagemTelegram);
			if (BooleanUtils.isFalse(test)) {
