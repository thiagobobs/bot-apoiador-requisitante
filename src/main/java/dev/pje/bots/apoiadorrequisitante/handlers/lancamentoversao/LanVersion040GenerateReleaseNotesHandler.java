...
//	private NewVersionReleasedSimpleCallTextModel newVersionSimpleCallModel;
	...
	public VersionReleaseNotes geraReleaseNotes(String gitlabProjectId, String version) {
		messages.clean();
...
//						String jiraProjectKey = issue.getFields().getProject().getKey();
						...
						if (jiraProjectKey != null) {
							...
							String jql = jiraService.getJqlIssuesFromFixVersion(version, true, jiraRelatedProjectKeys);
							List<JiraIssue> issues = jiraService.getIssuesFromJql(jql);
							
							if(issues != null && issues.size() > 0){
								releaseNotes = new VersionReleaseNotes();
	...
	//							}
								releaseNotes.setProject(issues.get(0).getFields().getProject().getName());
	...
	//								releaseNotes.setVersionType(issue.getFields().getTipoVersao().getValue());
								releaseNotes.setVersionType("Ordinária");
	//							}
	...
	//								releaseNotes.setUrl(issue.getFields().getUrlPublicacaoDocumentacao());
								releaseNotes.setUrl("https://docs.pje.jus.br/");
	//							}
