	...
	private void liberaVersaoJira(String jiraProjectKey, String appVersion) {
		...
		for (String relatedProjectKey : relatedProjectKeys) {
			...
			if (jiraVersion != null) {
				...
				messages.info("Versão " + appVersion + " do projeto " + relatedProjectKey + " finalizada em " + Utils.dateToStringPattern(Calendar.getInstance().getTime(), Utils.DATE_PATTERN_SIMPLE));
			} else {
				messages.info("Versão " + appVersion + " não encontrada no projeto " + relatedProjectKey);
			}
		}
	}
