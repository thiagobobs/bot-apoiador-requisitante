...
public class ReleaseNotesTextModel implements AbstractTextModel{
	...
	public String convert(MarkdownInterface markdown) {
		...
		if(releaseNotes != null && releaseNotes.getVersion() != null && releaseNotes.getVersionType() != null) {
			...
			markdownText.append(markdown.head2("Versão " + releaseNotes.getVersion()));
...
//				.append(" disponibilizou esta versão");
			...
			try {
				...
				markdownText
					.append("Versão disponibilizada em ")
...
//			}
			...
			markdownText
				.append(markdown.block("Visão Geral", String.join(" - ", contadorTipoIssue)));
...
//			String PJE_PROJECT_DOCS_URL = "https://docs.pje.jus.br/servicos-negociais/pje-legacy/pje-legacy.html";
			...
			markdownText.append(markdown.head4("Outras informações"));
			if (StringUtils.isNotEmpty(linkToVersion)) {
				markdownText.append(markdown.listItem("Link desta versão no jira: " + linkToVersion));
			}
...
//				.append(markdown.listItem("Para mais informações, acesse a documentação do projeto em " + markdown.link(DOCSURL)))
/**				.append(markdown.listItem("Para mais informações, acesse a documentação do projeto em " + markdown.link(PJE_PROJECT_DOCS_URL)))
				.append(markdown.highlight("TIP: Acompanhe as notícias do PJe em primeira-mão no canal (público) do telegram: " + markdown.link(TELEGRAM_CHANNEL_URL, "@" + TELEGRAM_CHANNEL_NAME)));**/
		}else {
