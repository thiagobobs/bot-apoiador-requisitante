...
//		String jql = "category in (PJE, PJE-CLOUD) AND fixVersion in (" + version + ")";
		String jql = "fixVersion = " + version;
		if(onlyClosed) {
			jql += " AND status in (Fechado, Resolvido, Feito)";
		}
...
//		jql += " ORDER BY project ASC, status ASC, priority DESC, key ASC";
		jql += " ORDER BY issuetype DESC, updated ASC";
...
//			telegramService.sendBotMessage(errorMesasge);
	...
	public void atualizarTribunaisRevisores(JiraIssue issue, List<String> novosTribunaisRevisores, Map<String, Object> updateFields) {
		JiraIssue issueDetalhada = recuperaIssueDetalhada(issue);
		...
		if (houveAlteracao) {
			...
			Map<String, Object> updateField = null;
			try {
				updateField = createUpdateObject(fieldName, novosTribunaisRevisores, "UPDATE");
			} catch (Exception e) {
	...
	public void atualizarResponsaveisRevisao(JiraIssue issue, List<JiraUser> novosUsuariosRevisores, Map<String, Object> updateFields) {
		JiraIssue issueDetalhada = recuperaIssueDetalhada(issue);
		...
		if (houveAlteracao) {
			...
			Map<String, Object> updateField = null;
			try {
				updateField = createUpdateObject(fieldName, novosUsuariosRevisores, "UPDATE");
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	private JiraVersion getProjectVersion(JiraIssue issue, String versionName, boolean createIfDoesNotExists) {
		...
		if(version == null && createIfDoesNotExists) {
			version = createProjectVersion(projectKey, versionName, null);
		}
		return version;
	}

	public void atualizarFixVersion(JiraIssue issue, String fixVersion, Map<String, Object> updateFields, boolean replaceValue) {
		JiraVersion version = getProjectVersion(issue, fixVersion, true);
		atualizarFixVersion(issue, version, updateFields, replaceValue);
	}

	public void atualizarFixVersion(JiraIssue issue,  JiraVersion fixVersion, Map<String, Object> updateFields, boolean replaceValue) {
		if (fixVersion != null) {
			JiraIssue issueDetalhada = recuperaIssueDetalhada(issue);
			...
			if(houveAlteracao || replaceValue) {
				...
				Map<String, Object> updateField = null;
				try {
					updateField = createUpdateObject(FIELD_FIX_VERSION, fixVersions, "UPDATE");
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	public void atualizarIntegradoNosBranches(JiraIssue issue, String branchName, Map<String, Object> updateFields, boolean replaceValue) {
	...
	public void atualizarIntegradoNosBranches(JiraIssue issue,  JiraVersion branch, Map<String, Object> updateFields, boolean replaceValue) {
		if (branch != null) {
			JiraIssue issueDetalhada = recuperaIssueDetalhada(issue);
			...
			if(houveAlteracao || replaceValue) {
				...
				Map<String, Object> updateField = null;
				try {
					updateField = createUpdateObject(FIELD_INTEGRADO_NOS_BRANCHES, branches, "UPDATE");
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	public void atualizarResponsavelRevisao(JiraIssue issue,  JiraUser usuario, Map<String, Object> updateFields) {
		if (usuario != null) {
			JiraIssue issueDetalhada = recuperaIssueDetalhada(issue);
			...
			if(houveAlteracao) {
				Map<String, Object> updateField = null;
				try {
					updateField = createUpdateObject(FIELD_RESPONSAVEL_REVISAO, usuario, "UPDATE");
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	public void atualizarAprovacoesRealizadas(JiraIssue issue,  Integer aprovacoesRealizadas, Map<String, Object> updateFields) {
		JiraIssue issueDetalhada = recuperaIssueDetalhada(issue);
		...
		if(houveAlteracao) {
			Map<String, Object> updateField = null;
			try {
				updateField = createUpdateObject(FIELD_APROVACOES_REALIZADAS, aprovacoesRealizadas, "UPDATE");
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public void atualizarBranchRelacionado(JiraIssue issue,  String branchName, Map<String, Object> updateFields, boolean replaceValue) {
		JiraIssue issueDetalhada = recuperaIssueDetalhada(issue);
		...
		if(houveAlteracao || replaceValue) {
			...
			Map<String, Object> updateField = null;
			try {
				updateField = createUpdateObject(FIELD_BRANCHES_RELACIONADOS, branchesRelacionados, "UPDATE");
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public void atualizarMRsAbertos(JiraIssue issue,  String mrAberto, Map<String, Object> updateFields, boolean replaceValue){
		JiraIssue issueDetalhada = recuperaIssueDetalhada(issue);
		...
		if(alterarCampo) {
			...
			Map<String, Object> updateField = null;
			try {
				updateField = createUpdateObject(FIELD_MRS_ABERTOS, mrsAbertos, "UPDATE");
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public void atualizarMRsAceitos(JiraIssue issue,  String mrAceito, Map<String, Object> updateFields, boolean replaceValue) {
		JiraIssue issueDetalhada = recuperaIssueDetalhada(issue);
		...
		if(houveAlteracao || replaceValue) {
			...
			Map<String, Object> updateField = null;
			try {
				updateField = createUpdateObject(FIELD_MRS_ACEITOS, mrsAceitos, "UPDATE");
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public void adicionarComentario(JiraIssue issue, String texto, Map<String, Object> updateFields) {
		if (StringUtils.isNotBlank(texto)) {
			Map<String, Object> updateField = null;
			try {
				updateField = createUpdateObject(FIELD_COMMENT, texto, "ADD");
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
...
//			telegramService.sendBotMessage(errorMesasge);
	...
	public JiraIssueTransition findTransitionByPropertyKey(JiraIssue issue, String transitionPropertyKey) {
		...
		JiraIssueTransitions transitions = this.recuperarTransicoesIssue(issue.getKey());
		if (transitions != null) {
...
//			telegramService.sendBotMessage(errorMesasge);
	...
	public void updateIssue(String issueKey, JiraIssueCreateAndUpdate issueUpdate) {
		if (issueKey != null && issueUpdate != null) {
			try {
				...
				logger.error("Erro ao atualizar a issue " + issueKey, e);
			}
		}
	}
	...
	public void sendTextAsComment(String issueKey, String text) {
		if (StringUtils.isNotBlank(text)) {
			try {
				jiraClient.sendComment(issueKey, new JiraComment(text));
			} catch (Exception e) {
				logger.error("Erro ao enviar comentário para issue " + issueKey, e);
			}
		}
	}
...
//	@Cacheable(cacheNames = "issue-transitions")
	public JiraIssueTransitions recuperarTransicoesIssue(String issueKey) {
		...
		try {
			...
			logger.error("Erro ao recuperar as transições da issue " + issueKey, e);
		}
		return transitions;
	}
...
//				telegramService.sendBotMessage(errorMesasge);
	...
	public void createVersionInRelatedProjects(String projectKey, String newVersion) {
		...
		if(relatedProjectKeys != null) {
			for (String relatedProjectKey : relatedProjectKeys) {
				...
				JiraVersion jiraVersion = createProjectVersionIfNotExists(relatedProjectKey, newVersion, versionDescription);
				if(jiraVersion != null) {
	...
	public void releaseVersionInRelatedProjects(String projectKey, String newVersion, String releaseDate, String versionDescription) {
		List<String> relatedProjectKeys = getJiraRelatedProjects(projectKey);
		if(relatedProjectKeys != null) {
			for (String relatedProjectKey : relatedProjectKeys) {
...
//					telegramService.sendBotMessage(errorMesasge);
	...
	public JiraVersion updateProjectVersion(JiraVersion version) {
...
//	@Cacheable(cacheNames = "project-versions")
	private List<JiraVersion> getProjectVersions(String projectKey) {
		List<JiraVersion> projectVersions = new ArrayList<>();

		try {
			Map<String, String> options = new HashMap<>();
			projectVersions = jiraClient.getAllVersions(projectKey, options);
		} catch (Exception e) {
			logger.error("Falha ao recuperar as versões cadastradas para o projeto " + projectKey, e);
		}
	...
	public JiraVersion createProjectVersionIfNotExists(String projectKey, String versionName, String description) {
		JiraVersion version = findProjecVersion(projectKey, versionName);

		if (version == null) {
			version = createProjectVersion(projectKey, versionName, description);
		}

		return version;
	}

	private JiraVersion createProjectVersion(String projectKey, String versionName, String description) {
		JiraVersion version = null;

		JiraProject project = this.getProject(projectKey);
		if (project != null) {
			...
			logger.info("Versão {} criada para o projeto {}", version.getName(), projectKey);
		}
...
//	@Cacheable(cacheNames = "project-detail")
	public JiraProject getProject(String projectKey) {
		...
		try {
			project = jiraClient.getProjectDetails(projectKey, Collections.emptyMap());
		} catch (Exception e) {
			logger.error("Falha ao recuperar informações sobre o projeto " + projectKey, e);
		}
...
//			telegramService.sendBotMessage(errorMesasge);
	...
	public JiraIssue getIssue(String mergeTitle, GitlabCommit lastCommit) {
		...
		String issueKey = Utils.getIssueKey(mergeTitle);
		if(StringUtils.isBlank(issueKey)) {
			issueKey = Utils.getIssueKey(lastCommitTitle);
		}
